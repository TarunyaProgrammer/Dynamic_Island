...
// A copy is included at /LICENSE in this repository (Beacon is itself Apache-2.0
// licensed, so this file's terms match the project's own root license).
// Attribution: THIRD_PARTY_NOTICES.md.
//
// Modified for Beacon: renamed `NookScreenLocator` -> `ScreenLocator` (the `Nook`
// prefix is redundant once the type lives in Beacon's own module; see CLAUDE.md's
// "no `Beacon` prefix" convention applied in reverse). Renamed `NookDisplayPreference`
...
// `NookPreferenceStorage`, a NookKit-only type. Beacon persists preferences through the
// `Defaults` package (see `Beacon/Core/Preferences.swift`), not a hand-rolled JSON
...
// `static func`/`init` -> internal) since both types are internal to the Beacon module,
// with no cross-module consumer the way NookKit had one. Added `nonisolated` -
// narrowly, not to the whole file - because Beacon's project settings default every
