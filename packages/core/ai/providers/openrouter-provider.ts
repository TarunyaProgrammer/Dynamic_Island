// packages/core/ai/providers/openrouter-provider.ts - Universal OpenRouter Provider
    ...
    try {
      ...
      const res = await this.fetchWithTimeout(
        this.baseUrl,
        {
          ...
          headers: {
            ...
            'HTTP-Referer': 'https://beacon.app',
            'X-Title': 'Beacon Desktop',
          },
          body: JSON.stringify({
            model: testModel,
      ...
      if (!res.ok) {
        ...
        return {
          success: false,
          latencyMs,
          error: `OpenRouter returned HTTP ${res.status}: ${errorText.slice(0, 120)}`,
          modelUsed: testModel,
        };
      }
