...
//
// Modified for Beacon: added `syntheticNotchWidth`, published `notchSize`/`menubarHeight`,
...
public final class Nook: ObservableObject, NookControllable {
    ...
    /// Modified for Beacon: width of the notch shape drawn on displays with no physical
    /// notch. Upstream hardcodes 300pt, which reads as conspicuously wider than a real
    /// notch (185-208pt). Beacon presents the notch layout on every Mac, so this has to be
    ...
    /// Modified for Beacon: exposed publicly. Beacon sizes its own compact slots against the
    ...
    func updateHoverState(_ hovering: Bool) {
        ...
        // Modified for Beacon: hover-driven transitions are gated on `.expandsOnHover`.
