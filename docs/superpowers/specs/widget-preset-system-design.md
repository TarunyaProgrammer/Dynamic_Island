# Beacon Widget Preset System — 永続設計記録
...
- 新機能追加 = BeaconWidget を実装して WidgetRegistry に register() するだけ
- タブ切り替え = プリセット切り替え（PresetStore.select(id:)）

## ウィジェットとは

Beacon の各機能はそれぞれ独立した「ウィジェット」として実装される。

```swift
// Beacon/Core/WidgetProtocol.swift
protocol BeaconWidget: Identifiable {
    var id: String { get }
...
```swift
// Beacon/Core/WidgetLayout.swift
...
| コンポーネント | ファイル | 状態 |
|--------------|---------|------|
| BeaconWidget protocol | `Beacon/Core/WidgetProtocol.swift` | 完成 |
| WidgetSize / WidgetPosition | `Beacon/Core/WidgetTypes.swift` | 完成 |
| WidgetRegistry | `Beacon/Core/WidgetRegistry.swift` | 完成 |
| WidgetPlacement / PresetLayout | `Beacon/Core/WidgetLayout.swift` | 完成 |
| PresetStore (CRUD + 永続化) | `Beacon/Core/PresetStore.swift` | 完成 |
| NowPlayingWidget (3サイズ) | `Beacon/UI/Cards/NowPlayingWidget.swift` | BeaconWidget 準拠 |
| AIUsageWidget (4サイズ) | `Beacon/UI/Cards/AIUsageWidget.swift` | BeaconWidget 準拠 |
| AppState の移行 | `Beacon/Core/AppState.swift` | 未完 — まだ IslandPreset を使用 |
| ExpandedIslandView | `Beacon/UI/ExpandedIslandView.swift` | 未完 — ハードコードレイアウト |
| PresetTabBar | `Beacon/UI/PresetTabBar.swift` | 未完 — IslandPreset.allCases 使用 |
