...
final class MenuBarController {
    ...
    private let logger: Logger = {
        var logger = Logger(label: "com.beacon.MenuBarController")
    ...
    private func configure() {
        ...
        button.image = NSImage(systemSymbolName: "bird", accessibilityDescription: "Beacon")
        button.image?.isTemplate = true

        let menu = NSMenu()
        menu.addItem(withTitle: "Settings…", action: #selector(openSettings), keyEquivalent: ",")
        menu.addItem(.separator())
        menu.addItem(withTitle: "Quit Beacon", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
