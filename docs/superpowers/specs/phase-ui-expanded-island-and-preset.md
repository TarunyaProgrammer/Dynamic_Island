...
```
Beacon/UI/ExpandedIslandView.swift  — ウィジェット配置をハードコード
Beacon/UI/PresetTabBar.swift        — プリセットタブをハードコード
Beacon/Core/AppState.swift          — IslandPreset enum が残存
```
...
| ファイル | 作業内容 |
|---------|---------|
| `Beacon/UI/ExpandedIslandView.swift` | switch/if ハードコードを `ForEach + WidgetRegistry` に置き換え |
| `Beacon/UI/PresetTabBar.swift` | 固定タブ定義を `presetStore.presets` iterate に差し替え |
| `Beacon/Core/AppState.swift` | `IslandPreset` enum を廃止、`PresetStore` に一本化 |
...
- `beaconSpring`: `.spring(response: 0.32, dampingFraction: 0.82)` — 標準遷移
- `beaconExpand`: `.spring(response: 0.35, dampingFraction: 0.86)` — 展開
- `beaconSubtle`: `.easeInOut(duration: 0.2)` — 微細変化
...
`Beacon/UI/SettingsView.swift` に「プリセット」タブを新規追加（既存タブを崩さない）
...
| ファイル | 役割 |
|---------|------|
| `Beacon/Core/PresetStore.swift` | プリセット CRUD + Defaults 永続化 |
| `Beacon/Core/WidgetLayout.swift` | `WidgetPlacement`, `PresetLayout` |
| `Beacon/Core/WidgetRegistry.swift` | ウィジェット登録・検索 |
| `Beacon/Core/WidgetProtocol.swift` | `BeaconWidget` プロトコル |

### PresetStore API（変更禁止・必ず使用）

```swift
// Beacon/Core/PresetStore.swift
...
| PresetStore API の完全性 | 移行前に `Beacon/Core/PresetStore.swift` を読み実際の API を確認する |
