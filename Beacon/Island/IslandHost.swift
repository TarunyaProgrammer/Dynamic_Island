...
private let logger = Logger(label: "com.beacon.IslandHost")

/// Owns the island: the vendored surface, the `NookBridge` that applies Beacon's policy to
...
final class IslandHost {
    ...
    init(appState: AppState) {
        ...
        // Replaces the bridge's default, which reaches for `NSScreen.preferredScreen`
    ...
    /// `.onTapGesture` on Beacon's own content, because in compact mode Beacon only supplies
    /// the two slots that flank the notch — the gap between them is drawn by the vendored
    /// view and is not Beacon's to attach a gesture to. In `.notch` chrome on a Mac with no
