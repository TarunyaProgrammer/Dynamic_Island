...
private let logger = Logger(label: "com.beacon.NookBridge")

/// Owns the vendored notch surface and everything Beacon has to do *around* it: honour
...
/// reasons: the surface, not Beacon, is about to become the source of truth for expansion
...
final class NookBridge {
    ...
    /// `NSScreen.preferredScreen`; the surface has the same seam, so the preference
    ...
    init(
        ...
        // Beacon owns collapse timing, so the surface must not compact itself the instant
    ...
    /// which has to live on the window because the notch gap between Beacon's two compact
    ...
    ///   hardware. Staying on the vendored value keeps Beacon aligned with upstream; see
    ...
    /// whatever was behind the panel rather than the solid black chrome Beacon wants (see
    ...
    ///
    /// The decision lives here, in Beacon; the assignment lives in
