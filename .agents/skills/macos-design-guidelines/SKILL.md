...
- Spring animation パラメータ: response 0.32, dampingFraction 0.82（標準）
- hover展開: 0.3秒ディレイ後にトリガー。即クリックでも展開
- 自動収納: マウス離脱3秒後。操作中はタイマーリセット
- 通知表示: ピルが一時的に膨張 -> 内容表示 -> 収縮（Dynamic Island的）

### Notch Adaptation Rules
- ノッチなしMacをfallback扱いにしない。floatingPillとして同等の体験を提供
- ノッチありの場合はノッチ幅に合わせたcompactサイズ
- 外部ディスプレイ（ノッチなし）でも自然に動作

### vibrancy / material Usage
- NSVisualEffectView `.ultraDark` materialをベースに
- 純粋黒ではなくmacOSらしい半透明vibrancy
- expanded時は微かなシャドウ追加

### Window Behavior
- `level: .statusBar + 1`（設定変更可能）
- `collectionBehavior: [.fullScreenAuxiliary, .stationary, .canJoinAllSpaces, .ignoresCycle]`
- compact時: ピル外はクリックパススルー
- expanded時: カード全体がイベント受け取り
