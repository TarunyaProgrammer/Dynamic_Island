...
/// in `BeaconTests`, and that the vendoring dependency stays pinned to one conformance
/// (`Nook: IslandSurfaceDriving`, below) instead of spreading through Beacon.
///
...
/// take Beacon's own vocabulary (`IslandChromeStyle`, a width, a boolean) and the
...
protocol IslandSurfaceDriving: AnyObject {
    ...
    /// Beacon sets this so `NookBridge` can honour the user's auto-collapse delay.
    ...
    /// of dipping through the hidden state on the way. Beacon turns this on: the island
    ...
    /// Switches the chrome between the pseudo-notch and floating looks. Takes Beacon's own
...
extension Nook: IslandSurfaceDriving {
    ...
    /// The single point where a Beacon chrome style becomes a vendored `NookPresentation`.
    ...
    /// The single point where a Beacon backdrop decision becomes a vendored `NookBackdrop`.
    func applyBackdrop(reduceTransparency: Bool) {
