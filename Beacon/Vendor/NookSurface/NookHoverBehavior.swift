...
//
// Modified for Beacon: added `.expandsOnHover` so hover-driven expansion can be turned
// off. Upstream expands unconditionally on hover with no way to opt out, which forecloses
// a design decision Beacon wants to keep open.
...
public struct NookHoverBehavior: OptionSet, Sendable {
    ...
    /// Modified for Beacon: hovering expands the surface, and leaving it compacts again.
    /// Without this option `isHovering` still updates, so a host can drive its own
    /// timing — which is what Beacon does to honour its auto-collapse delay.
