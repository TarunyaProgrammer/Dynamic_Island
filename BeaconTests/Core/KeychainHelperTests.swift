// BeaconTests/Core/KeychainHelperTests.swift
import Foundation
import Testing
...
struct KeychainHelperTests {
    private let testKey = "beacon-test-keychain-\(UUID().uuidString)"
    ...
    func loadMissingReturnsNil() {
        let loaded = KeychainHelper.load(forKey: "beacon-test-nonexistent-\(UUID())")
        #expect(loaded == nil)
    }
    ...
    func deleteNonExistentIsSafe() {
        ...
        KeychainHelper.delete(forKey: "beacon-test-no-such-key-\(UUID())")
    }
