...
Beacon is a macOS menubar-level overlay app inspired by iOS Dynamic Island. It provides a compact pill UI anchored to the top-center of the screen (or aligned with the MacBook notch) that expands into rich information cards for music playback, AI usage tracking, file staging, and developer status monitoring.
...
- Bundle ID: `com.tukuyomi032.beacon`
...
beacon/
  ...
  Resources/    — Assets.xcassets, Info.plist, Beacon.entitlements
```
...
- `NSScreen` extension: `beaconNotchSize` (CGSize), `isBuiltInDisplay` (Bool)
...
- Header: left "Beacon" + right card name
...
1. API key → Settings input → Keychain (`Security.framework`, service: `com.tukuyomi032.beacon.<provider>`)
...
- Path: `~/Library/Application Support/Beacon/Shelf/{uuid}-{filename}`
...
- Auth: Fine-grained PAT → Keychain `com.tukuyomi032.beacon.github`
...
- Custom sounds: MP3/WAV/AIFF → `~/Library/Application Support/Beacon/Sounds/`
- 30s max (UNNotificationSound limit)

### DesignSystem

- Colors: beaconBackground (.black.opacity(0.85)), beaconSuccess/Failure/Warning/Running/Idle
...
- Animations: `beaconSpring` (0.32/0.82), `beaconExpand` (0.35/0.86), `beaconSubtle` (0.2s easeInOut)
...
- `cask "beacon"`: GitHub Releases dmg download
- zap: Application Support/Beacon, Preferences plist, Caches
